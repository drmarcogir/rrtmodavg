...
(Burnham & Anderson 2003). Model averaging is performed on the 95% confidence set of models by starting with the 
...
by Burnham & Anderson (2003) and Cade (2015). First a 95% confidence set of models selected by starting with the 
models with the highest weight (Akaike or BIC weight) and adding to the model with the
...
The function `predrrt()` calculates model-averaged fitted values Firsta a 95% confidence set of models selected by starting with the models with the highest weight (Akaike or BIC weight) and adding to the model with the next highest weight until the cumulative sum of weights exceeds 0.95. Fitted values are calculated...
