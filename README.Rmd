...
Model-averaged coefficients are calculated as the weighted average of the coefficients across
...
The function `predrrt()` calculates model-averaged fitted values Firsta a 95% confidence set of models selected by starting with the models with the highest weight (Akaike or BIC weight) and adding to the model with the next highest weight until the cumulative sum of weights exceeds 0.95. Model-averaged fitted values are calculated as the weighted average of the fitted values of all the models across the confidence set, where weights are Aikaike or BIC weights. 
