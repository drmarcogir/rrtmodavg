...
(Burnham & Anderson 2003). Model averaging is performed on the 95% confidence set of models. 
A 95% confidence set of models selected by starting with the model displaying the 
highest weight (Akaike or BIC weight)
and adding to the model with the next highest weight until the cumulative sum of weights exceeds 0.95

by starting with the 
models with the highest weight (Akaike or BIC weight) and adding to the model with the
next highest weight until the cumulative sum of weights exceeds 0.95.
...
by Burnham & Anderson (2003) and Cade (2015). First a 95% confidence set of models is selected by starting with 
the highest weight (Akaike or BIC weight) and adding to the model with the
next highest weight until the cumulative sum of weights exceeds 0.95. Variable
importance is then calculated as sum of the model weights over all models including the
explanatory variable (Burnham & Anderson 2003) or an average of the ratio of absolute values of the t statistic  over all 
models including the explanatory variable for unstandardized predictors, weightned by 
Akaike or BIC weights (Cade 2015).
...
The function `predrrt()` calculates model-averaged fitted values Firsta a 95% confidence set of models selected by starting with the models with the highest weight (Akaike or BIC weight) and adding to the model with the next highest weight until the cumulative sum of weights exceeds 0.95. Fitted are the weighted average of the fitted values of all the models across the confidence set, where weights are Aikaike or BIC weights. 
